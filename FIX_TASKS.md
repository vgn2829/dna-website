...
| C7 | ResourcesPage: build the real feature (schema, routes, admin tab, public page) | backend + frontend/Resources | own PR (largest change) | `feat/resources-page` | done, then **decided not currently needed** after live-preview review (2026-07-31/08-01) — feature stays fully built and working (not deleted), left unlinked from nav as it already was, and annotated with `RESOURCES_PAGE_FEATURE` comments (same convention as `PALETTE_STUDIO_FEATURE`) across `ResourcesPage.tsx`, `routes.tsx`, `AppDataContext.tsx`, `backend/src/routes/resources.ts`, `backend/src/db/schema.ts`, and the AdminPage `ResourcesTab` — explaining it's kept in case a future need (e.g. a blog/articles feature) wants this exact submit-then-approve shape. Verified build/typecheck still pass after the comment-only changes. |
...
| DEP5 | Outdated Radix UI packages (each ~1 minor/major behind) | deps | flagged back — see note | — | **flagged, not upgraded** — same finding as DEP3, but bigger: all 26 pinned `@radix-ui/react-*` packages have zero real usage. The 26 shadcn wrapper components in `src/app/components/ui/*` that import them are never imported by any actual page/feature component — grepped every one individually. The only `components/ui/*` file actually used anywhere is `pixel-trail.tsx` (imported by `Hero.tsx`), and it doesn't touch Radix at all — it's a standalone canvas effect. The `card`/`btn-primary`/etc. class names seen throughout `AdminPage.tsx` and elsewhere are **plain CSS utility classes** defined in `theme.css`, unrelated to the shadcn `Card` *component* in `ui/card.tsx` — confirmed these are two different things, not a false negative in the grep. This entire kit is unused Figma/Make scaffold, same category as DEP3's MUI finding but the whole 26-package surface rather than one library. |
...
| Radix UI packages (various) | various, 1 major/minor behind | **FLAGGED — dead dependency (all 26)** | Not upgraded. The entire shadcn/Radix UI kit under `src/app/components/ui/*` is unused — only `pixel-trail.tsx` (no Radix dependency) is actually imported anywhere in the app. Same "should this be removed, not upgraded" call as DEP3, at a larger scale (26 packages, ~30 wrapper component files). Flagging rather than deciding — this is a much bigger removal than MUI's, worth a deliberate decision given the volume of files involved. |
...
| — | `chore/upgrade-radix` | DEP5 | not opened — flagged back, no upgrade performed (all 26 packages are dead dependencies) |
