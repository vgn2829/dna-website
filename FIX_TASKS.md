...
| DEP4 | Zod 3→4 | deps/backend | own PR, only if verified safe | `chore/upgrade-zod-4` | done — upgraded, zero source changes needed, verified (typecheck, 15/15 tests, build clean), PR opened |
...
| Zod | 3.25.76 → 4.4.3 | **UPGRADED** | Zero source changes required. Checked the two known Zod-4-breaking patterns specifically: `z.record(...)` was already called with the two-argument form (`z.record(z.string(), z.string())` in `settings.ts`) that v4 requires — v3's deprecated single-arg shorthand was never used here — and error details are read via `.issues` (`artworks.ts`), not the removed `.errors` alias. Manually verified `z.string().email()` still produces the same `{ message, path }` shape this codebase actually reads (the internal `code`/`origin`/`format` fields changed, but nothing here touches them). The pinned pre-upgrade version (3.25.76) was already a recent 3.x release that shipped much of v4's engine as opt-in, which likely explains the unusually clean jump. |
...
| #29 | `chore/upgrade-zod-4` | DEP4 | open |
