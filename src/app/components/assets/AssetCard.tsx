...
import { MoreHorizontal, Download, ExternalLink, Trash2, ImagePlus } from 'lucide-react';
import type { Asset } from '../../lib/api';
import { assetMetaLine, FAMILY_LABEL, fileFamily } from '../../lib/assetLibrary';
...
export function AssetCard({
  ...
  const typeLabel = asset.kind === 'image' ? 'Image' : FAMILY_LABEL[fileFamily(asset.extension)];

  const primaryLabel = canInsert
    ? `Insert ${asset.filename} onto the board`
    : asset.kind === 'image' ? `Open ${asset.filename}` : `Download ${asset.filename}`;
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '10px 12px 12px', minWidth: 0 }}>
        ...
        <p style={{
          margin: '3px 0 0', fontSize: 12, color: 'var(--color-ink-muted)', fontFamily: 'var(--font-body)',
            ...
            {asset.url && (
              <DropdownMenu.Item className="dna-menu-item" asChild>
                <a href={asset.url} target="_blank" rel="noopener noreferrer">
                  {asset.kind === 'image' ? <><ExternalLink size={15} /> Open image</> : <><Download size={15} /> Download</>}
                </a>
              </DropdownMenu.Item>
            )}
