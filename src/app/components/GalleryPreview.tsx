...
const ARTWORKS = [
  { id: 1, title: 'Abstract Composition', artist: 'Rahul Kumar',  domain: 'Illustrator',    accent: '#007AFF', featured: true  },
  { id: 2, title: 'Portrait Study',       artist: 'Priya Sharma', domain: 'Photoshop',       accent: '#BF5AF2', featured: true  },
  { id: 3, title: '3D Character',         artist: 'Amit Patel',   domain: 'Blender',         accent: '#FF375F', featured: false },
  { id: 4, title: 'UI Dashboard',         artist: 'Sneha Mehta',  domain: 'Figma',           accent: '#00D4FF', featured: false },
  { id: 5, title: 'Motion Graphics',      artist: 'Karan Verma',  domain: 'After Effects',   accent: '#FF9500', featured: false },
  { id: 6, title: 'Brand Identity',       artist: 'Anjali Reddy', domain: 'UI/UX',           accent: '#34C759', featured: false },
];

const FEATURED = ARTWORKS.filter(a => a.featured);

function ArtworkCard({ a, i }: { a: typeof ARTWORKS[0]; i: number }) {
  ...
  return (
    <section id="gallery" style={{ background: 'var(--color-canvas)', padding: '96px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        {/* Full grid — desktop/tablet only */}
        <div className="hidden md:block" style={{ marginBottom: 48 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
            {ARTWORKS.map((a, i) => <ArtworkCard key={a.id} a={a} i={i} />)}
          </div>
        </div>

        {/* Featured artworks — mobile only */}
        <div className="block md:hidden" style={{ marginBottom: 48 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
            {FEATURED.map((a, i) => <ArtworkCard key={a.id} a={a} i={i} />)}
          </div>
        </div>
