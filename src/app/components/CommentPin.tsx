...
function CommentPinImpl({ comment, x, y, replyCount, isOpen, isUnread, onClick }: CommentPinProps) {
  ...
  return (
    <button
      ...
      aria-label={`Comment thread by ${comment.authorName ?? comment.authorRoll}${replyCount > 0 ? `, ${replyCount} ${replyCount === 1 ? 'reply' : 'replies'}` : ''}${comment.resolvedAt ? ', resolved' : ''}`}
      aria-pressed={isOpen}
      style={{
        position: 'absolute',
        left: x, top: y,
        ...
        border: isOpen ? '2px solid var(--color-brand)' : '2px solid var(--color-surface-1)',
